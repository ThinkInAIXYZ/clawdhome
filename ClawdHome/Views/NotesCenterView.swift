...
struct NotesCenterView: View {
        ...
        if !store.appInstalled {
            items.append(
                NotesAlertItem(
                    id: "runtime-install",
                    icon: "shippingbox",
                    title: "LLM Wiki app bundle is missing",
                    detail: "Install or repair the managed runtime before checking note mappings.",
                    tone: .critical
                )
            )
    ...
    private var runtimeSummaryDetail: String {
        guard store.appInstalled else { return "Managed app bundle is missing." }
