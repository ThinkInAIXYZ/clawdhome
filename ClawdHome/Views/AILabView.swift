...
private struct AIToolCard: View {
    ...
    var body: some View {
            ...
            HStack(spacing: 12) {
                ZStack {
                    Circle()
                        .fill(
                            tool.status == .ready
                                ? tool.theme.mainColor.opacity(0.10)
        ...
        // 绑定统一设计语言中的高级玻璃拟态卡片，当不可用时降低卡片点击并赋予不同渐变底色
        .premiumCard(theme: tool.theme, isAvailable: tool.status == .ready)
    }
}
