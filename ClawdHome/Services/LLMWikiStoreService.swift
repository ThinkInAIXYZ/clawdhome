...
final class LLMWikiStoreService {
    ...
    func persistedGlobalModelStore() -> GlobalModelStore {
        let store = GlobalModelStore()
        store.load()
        return store
    }

    func persistedGlobalLLMConfigOptions() -> [LLMWikiGlobalLLMConfigOption] {
        globalLLMConfigOptions(from: persistedGlobalModelStore())
    }

    func persistedLLMConfigSelection() -> LLMWikiLLMConfigSelection? {
        LLMWikiLLMConfigSelectionStore.shared.load()
    }

    func savePersistedLLMConfigSelection(source: LLMWikiLLMConfigSource, optionID: String?, observedGlobalRevision: Int) {
    ...
    private func migrateLegacyStoreIfNeeded() throws {
        let legacyURL = URL(fileURLWithPath: LLMWikiPaths.legacyAppStatePath(for: NSUserName()))
