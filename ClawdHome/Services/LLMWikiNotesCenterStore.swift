...
extension Notification.Name {
    static let llmWikiConfigDidChange = Notification.Name("LLMWikiConfigDidChange")
}
...
final class LLMWikiNotesCenterStore {
    ...
    func refresh(users: [ManagedUser], helperClient: HelperClient, gatewayHub: GatewayHub) async {
        ...
        appInstalled = runtimeManager.isInstalled()
        let runtimeStatus = await runtimeManager.clipServerStatus()
        appRunning = runtimeStatus == "running" || runtimeManager.isRunning()
    ...
    func launchOrRestart(helperClient: HelperClient, users: [ManagedUser], gatewayHub: GatewayHub) async {
        ...
        do {
            ...
            if runtimeManager.isRunning() {
                ...
                statusMessage = "LLM Wiki 已接管现有运行中的 runtime"
            } else {
                try await runtimeManager.ensureRunning()
    ...
    func saveConfigs(users: [ManagedUser], helperClient: HelperClient, gatewayHub: GatewayHub, modelStore: GlobalModelStore) async {
        do {
            ...
            let restartedRuntime = try await restartRuntimeIfNeeded()
            NotificationCenter.default.post(name: .llmWikiConfigDidChange, object: nil)
            statusMessage = llmConfigSource == .global
                ? (restartedRuntime ? "LLM Wiki 已按全局模型配置写入并重启运行时" : "LLM Wiki 已按全局模型配置写入本地 store")
                : (restartedRuntime ? "LLM Wiki 配置已写入并重启运行时" : "LLM Wiki 配置已写入本地 store")
            await refresh(users: users, helperClient: helperClient, gatewayHub: gatewayHub)
        } catch {
            errorMessage = error.localizedDescription
        }
    ...
    private func restartRuntimeIfNeeded() async throws -> Bool {
        let runtimeStatus = await runtimeManager.clipServerStatus()
        let shouldRestart = runtimeManager.isRunning() || runtimeStatus == "running"
