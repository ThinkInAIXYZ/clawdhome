...
```bash
# 前置条件：Node.js 20+, Rust 1.91+
git clone https://github.com/nashsu/llm_wiki.git
cd llm_wiki
npm ci
npm run dev            # 前端开发模式
npm run build          # 生产前端构建
npm run check:rust     # 嵌入式运行时检查
npm run package:desktop # 前端构建 + release 运行时构建
```

### 自动化发布

GitHub Actions 现已收敛为标准三段式流程：

- `CI`：测试和编译检查
- `Package Bundles`：手动打包并上传 workflow artifacts，不创建 Release
- `Release`：基于版本 tag 的正式发版

完整流程、目标平台矩阵和所需 secrets 见 [Release Automation](release-automation.md)。
