...
```bash
# Prerequisites: Node.js 20+, Rust 1.91+
...
GitHub Actions now uses a standardized three-stage pipeline:

- `CI` for tests and compile checks
- `Package Bundles` for manual artifact builds without creating a release
- `Release` for tagged production releases

See [Release Automation](release-automation.md) for the full process, target matrix, and required secrets.
