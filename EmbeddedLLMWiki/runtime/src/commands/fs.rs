...
use crate::types::wiki::{FileNode, SourceDocumentState};
...
#[tauri::command]
pub fn list_source_documents(project_path: String) -> Result<Vec<SourceDocumentState>, String> {
    ...
    let source_root = project.join("raw").join("sources");
    if !source_root.exists() || !source_root.is_dir() {
        return Ok(vec![]);
    }

    let mut states = Vec::new();
    collect_source_documents(project, &source_root, &mut states)?;
...
fn collect_source_documents(
    project_root: &Path,
    dir: &Path,
    results: &mut Vec<SourceDocumentState>,
) -> Result<(), String> {
    ...
    for entry in entries {
        ...
        if path.is_dir() {
            collect_source_documents(project_root, &path, results)?;
            continue;
        }
        ...
        let ext = path
            ...
            .unwrap_or("")
            .to_lowercase();
        if !AUTO_INGEST_EXTS.contains(&ext.as_str()) {
            continue;
        }
        ...
        results.push(SourceDocumentState {
